local M = {
  ...
  keys = {
    ...
    { "m<CR>b", "<CMD>MarksListBuf<CR>", desc = "List Marks in File" },
    { "m<CR>i", "<CMD>BookmarksListAll<CR>", desc = "List Bookmarks" },
...
M.config = function(_, opts)
  ...
  require("which-key").register({
    dm = {
      ...
      A = { desc = "Delete Mark A" },
      ["<space>"] = { desc = "Delete Lowercase Marks in File" },
    },
    m = {
      name = "+marks/jumps",
  ...
  map_repeat("n", { "]m", "[m" }, { marks.next, marks.prev }, { "Next Mark (<M-n>)", "Prev Mark (<M-p>)" })
  map_repeat("n", { "<C-;>", "<C-,>" }, { marks.next, marks.prev }, { "Next Mark", "Prev Mark" })
