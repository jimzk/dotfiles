  ...
  "MunifTanjim/nui.nvim",
  {
    ...
    opts = {
      ...
      filter_rules = {
        ...
        bo = {
          filetype = { "NvimTree", "neo-tree", "notify", "aerial", "fidget", "edgy", "help" },
  ...
  return {
    {
      ...
      function()
        if is_neo_tree_shown() then
          ...
          require("neo-tree.command").execute({ toggle = true, action = "focus", dir = vim.loop.cwd() })
        end
      end,
      desc = "Open NeoTree",
    },
...

    {
      ...
      function()
        if is_neo_tree_shown() then
          vim.cmd("Neotree buffers action=focus")
        else
          require("neo-tree.command").execute({ source = "buffers", action = "show", toggle = false })
        end
      end,
      desc = "Buffer NeoTree",
    },
