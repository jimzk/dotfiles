...
local postfix_type = function(trig, type)
  return postfix({
    ...
    desc = type .. "<…>",
    -- match pattern: start with whitespace or <, and followed by alphanum.
    match_pattern = "[ <]+[%w]+$",
  }, {
    f(function(_, parent)
      local text = parent.snippet.env.POSTFIX_MATCH
...
return {
  ...
  postfix_format("prl", "println!"),
  postfix_format("print", "print!"),
  postfix_format("pr", "print!"),
  ...
  -- not, ref, refm, deref
  postfix({ trig = "!", desc = "!expr" }, fmt("!{}", { l(l.POSTFIX_MATCH) })),
  postfix({ trig = "&", desc = "&expr" }, fmt("&{}", { l(l.POSTFIX_MATCH) })),
  postfix({ trig = "&m", desc = "&mut expr" }, fmt("&mut {}", { l(l.POSTFIX_MATCH) })),
  postfix({ trig = "*", desc = "*expr" }, fmt("*{}", { l(l.POSTFIX_MATCH) })),
  postfix({ trig = "dbg", desc = "dbg!(expr)" }, fmt("dbg!({})", { l(l.POSTFIX_MATCH) })),
  postfix({ trig = "dbgr", desc = "dbg!(&expr)" }, fmt("dbg!(&{})", { l(l.POSTFIX_MATCH) })),
  postfix({ trig = "call", desc = "…(expr)" }, fmt("{}({})", { i(1), l(l.POSTFIX_MATCH) })),
